...
RDBMS is a Relational Database Managment System and SQL is a Structured Query Language.
So essentially RDBMS is the program itself that manages the data while SQL is the language used
to work with that data.

1. Why do tables need a `primary key`?
   Because there has to be a unique identifier to retrieve individual rows.
